        ...
        VStack {
            ...
            Grid(self.viewModel.cardsOnScreen) { card in
                ...
                CardView(viewModel: self.viewModel, card: card)
                    .padding(3)
                    .rotation3DEffect(Angle(degrees: card.isChosen ? 180 : 0), axis: (0,1,0))
                    .animation(.linear(duration: 1))
                    .animation(.interpolatingSpring(stiffness: card.isMatched == true ? 1 : 0, damping: card.isMatched == true ? 1 : 0))
                    .onTapGesture {
