...
struct CardView: View {
    ...
    private func cardView(size: CGSize) -> some View {
        ...
        let frontFrameSize: CGSize = CGSize(width: size.width * 0.9, height: size.height * 0.9)
        let frontFrameLine: CGFloat = min(size.height, size.width) * 0.03
        let cornerRadius: CGFloat = min(size.height, size.width) / 10
        return ZStack {
            RoundedRectangle(cornerRadius: cornerRadius) // 外框
                .stroke(lineWidth: frontFrameLine)
                .frame(width: frontFrameSize.width, height: frontFrameSize.height, alignment: .center)
            RoundedRectangle(cornerRadius: cornerRadius) // 匹配与否背板
                .fill(card.isMatched == true ? Color.orange : Color.gray)
                .opacity(card.isChosen && viewModel.cardsBeChosen.count == 3 ? 1 : 0)
